...
public class Exercise2 {
    ...
    public void mapEmployeesToCodesOfLetterTheirPositionsUsingMapHelper() {
        ...
        List<Integer>
                ...
                MapHelper.from(employees)
                         ...
                         .flatMap(position -> calcCodes(position))
                         .getMapped();
        // TODO               MapHelper.from(employees)
    ...
    private static class MapHelper<T> {
        ...
        /**
         * Создает объект для маппинга, передавая ему новый список, построенный на основе исходного. Для добавления в
         * новый список каждый элемент преобразовывается с использованием заданной функции. ([T], (T -> R)) -> [R]
         *
         * @param mapping Функция преобразования элементов.
         */
        public <R> MapHelper<R> map(Function<T, R> mapping) {
            ArrayList<R> rList = new ArrayList<>(source.size());
            for (T t : source) {
                rList.add(mapping.apply(t));
            }
//            throw new UnsupportedOperationException();
        ...
        /**
         * Создает объект для маппинга, передавая ему новый список, построенный на основе исходного. Для добавления в
         * новый список каждый элемент преобразовывается в список с использованием заданной функции. ([T], (T -> [R]))
         * -> [R]
         *
         * @param flatMapping Функция преобразования элементов.
         */
        public <R> MapHelper<R> flatMap(Function<T, List<R>> flatMapping) {
            List<R> rListInFlatMap = new ArrayList<>();

            for (T t : source) {
                rListInFlatMap.addAll(flatMapping.apply(t));
            }
//            throw new UnsupportedOperationException();
            return MapHelper.from(rListInFlatMap);
        }
    }
}
