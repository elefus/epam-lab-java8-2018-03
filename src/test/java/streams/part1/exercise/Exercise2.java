...
public class Exercise2 {
    ...
    public void findEmployeeWithMaximumDurationAtOnePosition() {
        ...
        Employee expected = employees.stream()
                                     .max(Comparator.comparing(employee -> employee.getJobHistory()
                                                                                   .stream()
                                                                                   .max(Comparator.comparingInt(JobHistoryEntry::getDuration))
                                                                                   .get(), Comparator.comparingInt(JobHistoryEntry::getDuration)))
                                     .get();
    ...
    public void calcTotalSalaryWithCoefficientWorkExperience() {
        ...
        Double expected = employees.stream().map(Employee::getJobHistory)
                                   .mapToDouble(value -> value.get(value.size() - 1).getDuration() > 3
                                           ? 75_000 * 1.2
                                           : 75_000)
                                   .sum();
