...
public class Exercise2 {
    ...
    public void employersStuffList() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                .flatMap(employee -> employee.getJobHistory().stream()
                                .map(entry -> new PersonEmployerPair(entry.getEmployer(), employee.getPerson())))
                                .collect(toMap(
    ...
    public void indexByFirstEmployer() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                .map(employee -> new PersonEmployerPair(employee.getJobHistory().get(0).getEmployer(), employee.getPerson()))
                .collect(toMap(
                        PersonEmployerPair::getEmployer,
                        positionPair -> new HashSet<>(Collections.singletonList(positionPair.getPerson())),
                        (o1, o2) -> {
