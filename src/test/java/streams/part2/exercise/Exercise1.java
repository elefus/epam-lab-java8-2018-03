...
public class Exercise1 {
    ...
    public void groupPersonsByFirstPositionUsingToMap() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                                                   .collect(toMap(e -> e.getJobHistory().get(0).getPosition(),
                                                           Exercise1::valueMapper, Exercise1::mergeFunction));
    ...
    private static Set<Person> valueMapper(Employee employee) {
        HashSet<Person> result = new HashSet<>();
        result.add(employee.getPerson());
        return result;
    }

    private static Set<Person> mergeFunction(Set<Person> left, Set<Person> right) {
        left.addAll(right);
        return left;
    }
